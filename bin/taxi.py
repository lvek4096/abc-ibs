...
def submit():	#Backend - takes inputs, sends to MySQL db
	...
	date_inp=date.get()
	time_inp=time.get()
	taxitype_inp=taxitype.get()

	#confirm=tk.Message(window,text='',width=500,font=fnt)
	#confirm.grid(column=1,row=9)

	l1=[id,start_inp,end_inp,date_inp,time_inp,taxitype_inp]
	if (not start_inp=='' and not start_inp.isspace()) and (not end_inp=='' and not end_inp.isspace()) and (not date_inp=='' and not date_inp.isspace()) and (not time_inp=='' and not time_inp.isspace()):
		if start_inp in locations and end_inp in locations:
			...
			sql='insert into taxi_bkgs values (%s,%s,%s,%s,%s,%s)'
			val=(id,start_inp,end_inp,date_inp,time_inp,taxitype_inp)
			cur.execute(sql,val)
			con.commit()
...
tk.Label(window,text='Taxi Type',font=fnt).grid(column=0,row=8,sticky=tk.E,padx=10,pady=10)
n=tk.StringVar()
taxitype=ttk.Combobox(window,textvariable=n,font=fnt,width=19)
taxitype.grid(column=1,row=8,sticky=tk.EW,padx=10,pady=10)
taxitype['values']=('Standart','XL','Premium')
taxitype.current(0)
#buttons
subimg=tk.PhotoImage(file='monoico/icon-511.png')
btn=tk.Button(window,font=fnt,text='Submit',image=subimg,command=submit);btn.grid(column=1,row=12,padx=10,pady=10)
btn.image=subimg
...
btn3=tk.Button(window,font=fnt,image=clsimg,command=close);btn3.grid(column=0,row=13,padx=10,pady=10,sticky=tk.SW)
btn3.img=clsimg
