						...
						if (not uname.get()=='' and not uname.get().isspace()) and (not npass.get()=='' and not npass.get().isspace()):
							if uname.get() in users_list:
								...
								if confirm == True:
									...
									cur.execute(sql,val)
									con.commit()
									messagebox.showinfo('','Password for '+uname.get()+' changed.',parent=passwd_win)
									passwd_win.destroy()
								else:
									messagebox.showinfo('','Password for '+uname.get()+' has not been changed..\nThe database has not been modified.',parent=passwd_win)
							else:
							...
							if b not in users:
								if '@' in reg_email_inp and '.' in reg_email_inp:
									if len(reg_num_inp) == 10:
										...
										messagebox.showinfo('','The new user '+reg_fname_inp+' has been successfully registered.',parent=reg_win)
										reg_win.destroy()
									else:
								...
								messagebox.showerror('Error','Username '+reg_uname_inp+' already exists.',parent=reg_win)
						else:
							messagebox.showerror('Error','Please do not leave any fields blank.',parent=reg_win)
